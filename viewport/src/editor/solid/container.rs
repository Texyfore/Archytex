...
impl SolidContainer {
    ...
    pub fn export(&self) -> mdl::Model {
        mdl::Model {
            points: self.points.dump(|point| mdl::Point {
            ...
            faces: self.faces.dump(|face| mdl::Face {
                points: face.quad.map(|p| p as u32),
                texture_id: mdl::TextureID(face.texture),
            }),
            solids: self.solids.dump(|solid| mdl::Solid {
                faces: solid.faces.map(|f| f as u32),
            }),
        }
    }
}
