...
use crate::{textures::{TextureID, texture::Texture}, utilities::math::Vec3};
...
pub fn load(base: &str, name: &str) -> Result<Texture> {
    ...
    let image = read_first_rgba_layer_from_file(path, |size, _|{
        Texture::new(size.x() as u32, size.y() as u32)
